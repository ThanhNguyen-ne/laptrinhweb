...
$query = "SELECT * FROM san_pham 
          ...
          WHERE san_pham_loai.loai_san_pham_id = 3 AND san_pham.so_luong_ton > 0 
          ORDER BY $order_by 
        ...
        <div class="main-content">
            <div class="product-list">
                        ...
                        <?php foreach ($productList as $product): ?>
                            <div class="productCard" id="<?= $product['id'] ?>" onclick="redirectToDetail(<?= $product['id'] ?>)">
                                ...
                                <div class="product-buttons">
                                    <a href="yensaosanestkhanhhoa.php?add_to_cart=<?= $product['id'] ?>" class="btn-cart"><i class="fa-solid fa-cart-shopping"></i></a>
                ...
                <ul class="listPage">
                    <?php
                    // Pagination controls
                    $total_products_query = "SELECT COUNT(*) AS total FROM san_pham_loai WHERE loai_san_pham_id = 3";
                    $total_products_result = $conn->query($total_products_query);
                    $total_products = $total_products_result->fetch_assoc()['total'];
                    $total_pages = ceil($total_products / $limit);

                    if ($thisPage > 1) {
                        echo '<li><a href="yensaosanestkhanhhoa.php?page=' . ($thisPage - 1) . '&sort=' . $sort . '">TRƯỚC</a></li>';
                    }

                    // Page number links
                    for ($i = 1; $i <= $total_pages; $i++) {
                        $active = $i == $thisPage ? 'class="active"' : '';
                        echo '<li ' . $active . '><a href="yensaosanestkhanhhoa.php?page=' . $i . '&sort=' . $sort . '">' . $i . '</a></li>';
                    }

                    // Next page link
                    if ($thisPage < $total_pages) {
                        echo '<li><a href="yensaosanestkhanhhoa.php?page=' . ($thisPage + 1) . '&sort=' . $sort . '">SAU</a></li>';
                    }
                    ?>
                </ul>
            </div>
